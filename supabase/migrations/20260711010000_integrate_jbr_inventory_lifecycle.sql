-- Sprint 6: movimento de estoque atômico por JBR.
-- Checklist registra quantidades; liberação e retorno são os únicos pontos que alteram saldo.
...
-- Transição segura: as quantidades dos checklists antigos já afetaram o saldo
-- pelo trigger legado. Criamos a fotografia sem baixar o estoque novamente.
...
BEGIN
  ...
  -- Idempotência: nunca baixa o mesmo embarque duas vezes.
  IF v_service.logistics_inventory_dispatched_at IS NULL THEN
    -- Trava e valida o saldo agregado de cada item antes de qualquer baixa.
    FOR v_resource IN
      SELECT
...
BEGIN
  ...
  -- Fonte oficial: fotografia imutável feita na liberação logística.
  ...
  -- Compatibilidade com JBRs liberados antes da Sprint 6.
  IF NOT EXISTS (
...
BEGIN
  ...
  -- Idempotência: nunca aplica o mesmo retorno duas vezes.
